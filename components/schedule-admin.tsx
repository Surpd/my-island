...
function LessonInspector({ lesson, api, groups, teachers, students, lessons, subjects, saveStatus, onClose, onSave, onDelete }: { lesson: Lesson; api: AdminApi; groups: DirectoryOption[]; teachers: DirectoryOption[]; students: DirectoryOption[]; lessons: Lesson[]; subjects: string[]; saveStatus: SaveStatus; onClose: () => void; onSave: (lesson: Lesson) => void; onDelete: (lesson: Lesson) => void }) {
  ...
  const audienceLabel = audienceMode === 'available_slot' ? `Свободные ученики ${draft.grade} класса` : audienceMode === 'remaining' ? `Остаток деления «${partition}»` : audienceMode === 'grade' ? `Весь ${draft.grade} класс` : selectedGroups.join(' + ') || 'Аудитория не выбрана';
  const makeRule = () => {
...
function applyDraftChanges(base: Lesson[], changes: Item[], groups: DirectoryOption[], teachers: DirectoryOption[], students: DirectoryOption[]): Lesson[] {
  ...
  for (const change of changes) {
    ...
    const audienceMode: AudienceMode = rule.kind === 'whole_grade' ? 'grade' : rule.kind === 'base_class' ? 'base_class' : rule.kind === 'remaining' ? 'remaining' : rule.kind === 'available_slot' ? 'available_slot' : rule.kind === 'students' ? 'students' : 'groups';
...
export function ScheduleAdmin({ api }: { api: AdminApi }) {
  ...
  const deleteLesson = (lesson: Lesson) => { if (!window.confirm(`Удалить «${lesson.subject}» из черновика?`)) return; const blockKey = String(lesson.block?.block_key || lesson.id); const change = { operation: 'delete', block_key: blockKey, assignment_index: lesson.assignmentIndex ?? 0, manual: true }; setLessons((items) => items.filter((item) => item.id !== lesson.id)); setSelectedId(''); setDraftChanges((items) => { const previous = items.find((item) => sameChange(item, change)); const rest = items.filter((item) => !sameChange(item, change)); return previous?.change_kind === 'added' ? rest : [...rest, change]; }); generationRef.current += 1; setDraftDirty(true); dirtyRef.current = true; setSaveStatus('saving'); };
  const publish = async () => {
    ...
    try {
      ...
      setNotice(`Опубликовано и проверено повторным чтением · версия ${publishedId}`);
    } catch (error: any) {
