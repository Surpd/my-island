...
type AudienceMode = 'groups' | 'base_class' | 'grade' | 'remaining' | 'students';
type Lesson = { id: string; day: number; date: string; period: number; start: string; end: string; grade: number; subject: string; teacher: string; room: string; groups: string[]; groupIds?: string[]; teacherIds?: string[]; audienceMode: AudienceMode; audienceLabel: string; students: string[]; studentIds?: string[]; audienceRule?: Item; teacherColor: string; warning?: string; changed?: boolean; cancelled?: boolean; block?: Item };
type DirectoryOption = { id: string; name: string; meta?: string };
...
const isNoLesson = (subject: string) => /^no[\s_-]*lessons?$/i.test(String(subject || '').trim());
const SUBJECT_OPTIONS = [
  ...
  'Литература', 'Математика', 'Музыка', 'Настоящее', 'Обществознание', 'Пластика', 'Проект',
...
function assignmentToLesson(block: Item, assignment: Item, index: number): Lesson {
  ...
  const audienceMode: AudienceMode = ruleKind === 'remaining' || ruleKind === 'complement' ? 'remaining' : ruleKind === 'whole_grade' ? 'grade' : ruleKind === 'base_class' ? 'base_class' : ruleKind === 'students' ? 'students' : 'groups';
  const start = String(block.start_time || '09:00').slice(0, 5);
  return { id: `${String(block.block_key || date)}-${index}`, day, date, period: periodForStart(start), start, end: String(block.end_time || '09:45').slice(0, 5), grade: Number(String(block.grade_scope || '9').split(',')[0]) || 9, subject: String(assignment.activity || 'Занятие'), teacher, room: String(assignment.room || '—'), groups, groupIds: (assignment.canonical_group_ids || []).map(String), teacherIds: (assignment.teacher_ids || []).map(String), audienceMode, audienceRule: rule, audienceLabel: audienceMode === 'remaining' ? `Все остальные в делении «${String(rule.partition_dimension || 'учебная группа')}»` : groups.join(' + ') || `Весь ${block.grade_scope || 9} класс`, students: sample.map((item: Item) => String(item.name || item.id)).filter(Boolean), studentIds: sample.map((item: Item) => String(item.id)).filter(Boolean), teacherColor: TEACHER_COLORS[teacherIndex], warning: block.issues?.length ? `${block.issues.length} требуют внимания` : undefined, changed: String(block.change_classification || 'UNCHANGED') !== 'UNCHANGED', block };
}
...
function LessonCard({ lesson, compact = false, selected, onClick }: { lesson: Lesson; compact?: boolean; selected?: boolean; onClick: () => void }) {
  return <button type="button" className={`studio-lesson ${compact ? 'is-compact' : ''} ${selected ? 'is-selected' : ''} ${lesson.warning ? 'has-warning' : ''}`} style={{ '--teacher-color': lesson.teacherColor } as CSSProperties} onClick={onClick}><span className="studio-lesson__icon"><SubjectIcon subject={lesson.subject} size={compact ? 15 : 18} /></span><span className="studio-lesson__body"><strong>{displaySubject(lesson.subject)}</strong><small>{lesson.audienceLabel}</small>{compact ? <small>{lesson.period}-й урок · {BELL_SCHEDULE[lesson.period - 1]?.start}–{BELL_SCHEDULE[lesson.period - 1]?.end}</small> : <small>{lesson.teacher} · {lesson.room}</small>}</span>{lesson.changed ? <span className="studio-lesson__change" title="Изменено относительно шаблона" /> : null}{lesson.warning ? <AlertTriangle className="studio-lesson__warning" size={14} /> : null}</button>;
}
...
function LessonInspector({ lesson, api, groups, teachers, students, subjects, saveStatus, onClose, onSave, onDelete }: { lesson: Lesson; api: AdminApi; groups: DirectoryOption[]; teachers: DirectoryOption[]; students: DirectoryOption[]; subjects: string[]; saveStatus: SaveStatus; onClose: () => void; onSave: (lesson: Lesson) => void; onDelete: (lesson: Lesson) => void }) {
  ...
  const availableGroups = useMemo(() => groups.map((item) => item.name), [groups]);
  const availableTeachers = useMemo(() => teachers.map((item) => item.name), [teachers]);
  ...
  const audienceLabel = audienceMode === 'remaining' ? `Все остальные в делении «${partition}»` : audienceMode === 'grade' ? `Весь ${draft.grade} класс` : selectedGroups.join(' + ') || 'Аудитория не выбрана';
  const makeRule = () => {
    ...
    return { kind: audienceMode === 'grade' ? 'whole_grade' : audienceMode, grade: String(draft.grade), group_ids: audienceMode === 'groups' || audienceMode === 'base_class' ? groupIds : [], partition_dimension: audienceMode === 'remaining' ? `instructional:${draft.grade}:${subjectKey}` : '', include_student_ids: includeIds, exclude_student_ids: excludeIds };
  };
  const save = () => { const rule = makeRule(); onSave({ ...draft, audienceMode, groups: audienceMode === 'remaining' || audienceMode === 'students' ? [] : selectedGroups, groupIds: rule.group_ids, studentIds: rule.include_student_ids, audienceRule: rule, audienceLabel, students: preview, changed: true, warning: undefined }); };
  useEffect(() => {
    const rule = makeRule();
    const timer = window.setTimeout(() => api<Item>('/api/admin/schedule/audience-preview', { method: 'POST', body: JSON.stringify({ audience: rule }) }).then((result) => setRemotePreview((result.students || []).map((item: Item) => String(item.display_name || item.name || item.id)))).catch(() => setRemotePreview(null)), 250);
    return () => window.clearTimeout(timer);
  }, [api, audienceMode, draft.grade, excluded, groups, partition, selectedGroups, students]);
  useEffect(() => {
  ...
  return <aside className="studio-inspector" aria-label="Редактор урока"><header><div className="studio-inspector__title"><span className="studio-subject-mark" style={{ '--teacher-color': draft.teacherColor } as CSSProperties}><SubjectIcon subject={draft.subject} size={22} /></span><div><h2>{displaySubject(draft.subject)}</h2><p>{draft.grade} класс · {DAYS[draft.day]?.full}, {draft.period}-й урок · {BELL_SCHEDULE[draft.period - 1]?.start}–{BELL_SCHEDULE[draft.period - 1]?.end}</p></div></div><button type="button" className="studio-icon-button" onClick={onClose} aria-label="Закрыть"><PanelRightClose size={19} /></button></header>
    ...
    {tab === 'main' ? <div className="studio-inspector__content"><section className="studio-form-section"><h3>Занятие</h3><label>Предмет<input list="schedule-subject-options" placeholder="Выберите или введите предмет" value={displaySubject(draft.subject)} onChange={(event) => setDraft({ ...draft, subject: event.target.value === 'Нет урока' ? 'NO_LESSON' : event.target.value })} /><datalist id="schedule-subject-options">{subjects.map((subject) => <option key={subject} value={subject} />)}</datalist></label><label>Урок<select value={draft.period} onChange={(event) => { const period = Number(event.target.value); const slot = BELL_SCHEDULE[period - 1]; setDraft({ ...draft, period, start: slot.start, end: slot.end }); }}>{BELL_SCHEDULE.map((slot, index) => <option key={index + 1} value={index + 1}>{index + 1}-й · {slot.start}–{slot.end}</option>)}</select></label><label>Преподаватель<select value={draft.teacher} onChange={(event) => setDraft({ ...draft, teacher: event.target.value })}>{availableTeachers.map((name) => <option key={name}>{name}</option>)}</select></label><label>Кабинет<input value={draft.room} onChange={(event) => setDraft({ ...draft, room: event.target.value })} /></label></section>
      <section className="studio-form-section studio-audience-builder"><div className="studio-section-heading"><div><h3>Кто идёт на урок</h3><p>Аудитория хранится как явное правило этой версии.</p></div><Users size={18} /></div><div className="studio-audience-types">{([['groups', 'Группы'], ['base_class', 'Базовый класс'], ['grade', 'Вся параллель'], ['remaining', 'Все остальные'], ['students', 'Ученики']] as const).map(([value, label]) => <button type="button" key={value} className={audienceMode === value ? 'is-active' : ''} onClick={() => { setAudienceMode(value); setSelectedGroups([]); }}>{label}</button>)}</div>
        ...
        {audienceMode === 'remaining' ? <label>Деление<select value={partition} onChange={(event) => setPartition(event.target.value)}><option>Английский</option><option>Математика</option><option>ОГЭ</option></select><span className="studio-field-hint">Включит учеников параллели, не назначенных в другие группы этого деления.</span></label> : null}
...
function ImportReview({ api, onClose, onApply }: { api: AdminApi; onClose: () => void; onApply: (preview: Item) => Promise<void> }) {
  ...
  useEffect(() => { let cancelled = false; api<Item>(`/api/admin/schedule/preview?week_start=${WEEK_START}`, { method: 'POST' }).then((result) => { if (cancelled) return; if (result.status !== 'preview') throw new Error(String(result.message || 'Импорт недоступен')); setPreview(result); setStep('review'); }).catch((reason) => { if (!cancelled) { setError(String(reason?.message || 'Не удалось прочитать Google Sheets')); setStep('review'); } }); return () => { cancelled = true; }; }, [api]);
  const counts = preview?.diff_counts || {}; const changes: Item[] = preview?.draft_changes || []; const warnings: Item[] = preview?.warnings || [];
  return <div className="studio-modal-backdrop"><section className="studio-import-modal"><header><div><p>ИМПОРТ ИЗ GOOGLE SHEETS</p><h2>{step === 'done' ? 'Изменения добавлены в черновик' : 'Проверка недели 21–25 сентября'}</h2></div><button type="button" className="studio-icon-button" onClick={onClose}><X size={19} /></button></header>{step === 'loading' || step === 'applying' ? <div className="studio-import-loading"><RefreshCw size={24} className="admin-spin" /><strong>{step === 'applying' ? 'Сохраняем предложения в черновик…' : 'Читаем таблицу и сравниваем с текущей версией…'}</strong><span>Опубликованное расписание не меняется без подтверждения версии.</span></div> : step === 'done' ? <div className="studio-import-done"><span><Check size={28} /></span><h3>Черновик недели обновлён</h3><p>Предложения импорта сохранены на сервере. Ручные назначения аудитории сохранены или отмечены для проверки при изменении исходных ячеек.</p><button type="button" className="studio-primary-button" onClick={onClose}>Продолжить редактирование</button></div> : error ? <div className="studio-import-done"><span><AlertTriangle size={28} /></span><h3>Предпросмотр недоступен</h3><p>{error}</p><button type="button" className="studio-secondary-button" onClick={onClose}>Закрыть</button></div> : <><div className="studio-import-summary"><article className="is-good"><strong>{Number(counts.UNCHANGED || 0)}</strong><span>занятий совпадает</span></article><article><strong>{changes.length}</strong><span>изменения определены</span></article><article className="is-warn"><strong>{warnings.length}</strong><span>требуют проверки</span></article></div><div className="studio-import-list"><div className="studio-import-list__head"><strong>Изменения</strong><span>Предпросмотр — опубликованная версия не изменится</span></div>{changes.slice(0, 12).map((change) => { const lesson = change.lesson || {}; return <article key={change.block_key} className={change.review_required ? 'is-attention' : ''}><span className={`studio-import-status ${change.review_required ? 'is-warn' : 'is-change'}`}>{change.review_required ? 'Проверить' : String(change.change_kind || change.operation)}</span><div><strong>{displaySubject(String(lesson.activity || change.block_key))}</strong><p>{lesson.start_time ? `${lesson.start_time}–${lesson.end_time || ''}` : 'Занятие отменено'} · {lesson.grade ? `${lesson.grade} класс` : ''}</p>{change.review_reason ? <button type="button">{change.review_reason}</button> : null}</div>{change.review_required ? <AlertTriangle size={17} /> : <Check size={17} />}</article>; })}{!changes.length ? <div className="studio-import-empty">Изменений относительно опубликованной версии нет.</div> : null}</div><footer><button type="button" className="studio-secondary-button" onClick={onClose}>Отмена</button><button type="button" className="studio-primary-button" disabled={!changes.length} onClick={() => { if (!preview) return; setStep('applying'); void onApply(preview).then(() => setStep('done')).catch(() => setStep('review')); }}><Save size={16} /> Применить к черновику</button></footer></>}</section></div>;
}
...
function lessonToChange(lesson: Lesson, groups: DirectoryOption[], teachers: DirectoryOption[], students: DirectoryOption[]): Item {
  ...
  return { operation: 'upsert', block_key: String(lesson.block?.block_key || lesson.id), manual: true, audience_changed: true, lesson: { activity: lesson.subject, weekday: lesson.day, start_time: lesson.start, end_time: lesson.end, grade: String(lesson.grade), teacher_ids: teacherId ? [String(teacherId)] : [], teacher_names: [lesson.teacher], room: lesson.room, source_cells: lesson.block?.source_cells || [], source_identity: lesson.block?.source_provenance || {}, audience } };
}
...
function applyDraftChanges(base: Lesson[], changes: Item[], groups: DirectoryOption[], teachers: DirectoryOption[], students: DirectoryOption[]): Lesson[] {
  ...
  for (const change of changes) {
    const index = result.findIndex((lesson) => String(lesson.block?.block_key || lesson.id) === String(change.block_key));
    ...
    const audienceMode: AudienceMode = rule.kind === 'whole_grade' ? 'grade' : rule.kind === 'base_class' ? 'base_class' : rule.kind === 'remaining' ? 'remaining' : rule.kind === 'students' ? 'students' : 'groups';
    ...
    const next: Lesson = { ...(index >= 0 ? result[index] : seed), id: index >= 0 ? result[index].id : String(change.block_key), day: Number(raw.weekday || 0), date: `${WEEK_START}`, period, start, end, grade: Number(raw.grade || 9), subject: String(raw.activity || 'Занятие'), teacher: teacherName, teacherIds, room: String(raw.room || ''), groups: groupNames, groupIds, students: studentNames, studentIds, audienceMode, audienceRule: rule, audienceLabel: audienceMode === 'remaining' ? `Все остальные в делении «${String(rule.partition_dimension || '').split(':').pop() || 'группа'}»` : audienceMode === 'grade' ? `Весь ${raw.grade} класс` : groupNames.join(' + '), changed: true, warning: change.review_required ? String(change.review_reason) : undefined };
...
export function ScheduleAdmin({ api }: { api: AdminApi }) {
  ...
  const saveLesson = (next: Lesson) => { const slot = BELL_SCHEDULE[next.period - 1] || BELL_SCHEDULE[0]; const normalized = { ...next, start: slot.start, end: slot.end }; setLessons((items) => items.map((item) => item.id === normalized.id ? normalized : item)); const change = lessonToChange(normalized, groups, teachers, students); setDraftChanges((items) => [...items.filter((item) => String(item.block_key) !== String(change.block_key)), change]); generationRef.current += 1; setDraftDirty(true); dirtyRef.current = true; setSaveStatus('saving'); };
  const addLesson = (dayIndex: number, gradeNumber: number, period = 1) => { const slot = BELL_SCHEDULE[period - 1] || BELL_SCHEDULE[0]; const id = `manual-${crypto.randomUUID()}`; const lesson: Lesson = { id, day: dayIndex, date: WEEK_START, period, start: slot.start, end: slot.end, grade: gradeNumber, subject: 'Новое занятие', teacher: '', room: '', groups: [], audienceMode: 'grade', audienceLabel: `Весь ${gradeNumber} класс`, students: [], audienceRule: { kind: 'whole_grade', grade: String(gradeNumber), group_ids: [], partition_dimension: '', include_student_ids: [], exclude_student_ids: [] }, teacherColor: TEACHER_COLORS[0], changed: true }; setLessons((items) => [...items, lesson]); setSelectedId(id); const change = { ...lessonToChange(lesson, groups, teachers, students), change_kind: 'added' }; setDraftChanges((items) => [...items, change]); generationRef.current += 1; setDraftDirty(true); dirtyRef.current = true; setSaveStatus('saving'); };
  const deleteLesson = (lesson: Lesson) => { if (!window.confirm(`Удалить «${lesson.subject}» из черновика?`)) return; const blockKey = String(lesson.block?.block_key || lesson.id); setLessons((items) => items.filter((item) => item.id !== lesson.id)); setSelectedId(''); setDraftChanges((items) => [...items.filter((item) => String(item.block_key) !== blockKey), { operation: 'delete', block_key: blockKey, manual: true }]); generationRef.current += 1; setDraftDirty(true); dirtyRef.current = true; setSaveStatus('saving'); };
  const publish = async () => { setSaveStatus('saving'); try { const result = await api<Item>('/api/admin/schedule/draft/publish', { method: 'POST', body: JSON.stringify({ scope_kind: layer, scope_key: scopeKey, expected_revision: revisionRef.current, comment: 'Опубликовано из Schedule Studio' }) }); setDraftChanges([]); setDraftRevision(0); revisionRef.current = 0; setDraftDirty(false); dirtyRef.current = false; setSaveStatus('saved'); setShowPublish(false); setNotice(`Создана новая неизменяемая версия · ${String(result.published_id).slice(0, 12)}…`); } catch (error: any) { setSaveStatus(error?.status === 409 ? 'conflict' : 'error'); } };
  ...
  return <div className={`schedule-studio ${selected ? 'has-inspector' : ''}`}><section className="studio-main"><header className="studio-topbar"><div><p>MY ISLAND · УПРАВЛЕНИЕ ШКОЛОЙ</p><div className="studio-title-row"><h1>Расписание</h1><span className="studio-title-note">Неделя без хаоса <Sparkles size={14} /></span></div></div><div className="studio-topbar__actions"><button type="button" className="studio-secondary-button" onClick={() => setShowVersions(true)}><History size={16} /> История</button><button type="button" className="studio-import-button" onClick={() => layer === 'week' ? setShowImport(true) : setNotice('Импорт из Google Sheets сейчас обновляет только неделю. Шаблон можно изменить вручную и опубликовать новой версией; отдельный импорт шаблона пока не реализован.')}><CloudDownload size={17} /> Импорт из Google Sheets</button></div></header>
    ...
    <div className="studio-context-bar"><div className="studio-layer-switch"><button type="button" className={layer === 'week' ? 'is-active' : ''} onClick={() => switchLayer('week')}>Неделя</button><button type="button" className={layer === 'template' ? 'is-active' : ''} onClick={() => switchLayer('template')}>Шаблон</button></div><span>{draftChanges.length ? `Черновик · ${draftChanges.length} изменений${draftUpdatedAt ? ` · ${new Date(draftUpdatedAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}` : ''}` : layer === 'template' ? `Шаблон ${baseVersion}` : '21–25 сентября · опубликованная версия'}</span><div className={`studio-autosave is-${saveStatus}`}>{saveStatus === 'saving' ? <><RefreshCw size={14} className="admin-spin" /> Сохраняем…</> : saveStatus === 'error' ? <><AlertTriangle size={14} /> Не удалось сохранить</> : saveStatus === 'conflict' ? <><AlertTriangle size={14} /> Расписание изменилось в другой сессии</> : draftChanges.length ? <><Check size={14} /> Черновик сохранён{draftUpdatedAt ? ` · ${new Date(draftUpdatedAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}` : ''}</> : <><Check size={14} /> Сохранено</>}</div>{saveStatus === 'conflict' ? <div className="studio-conflict-actions"><button type="button" onClick={() => window.location.reload()}>Обновить данные</button><button type="button" onClick={() => setNotice('Сравнение: локальные изменения не отправлены; сервер содержит более новую ревизию черновика.')}>Сравнить изменения</button></div> : null}{draftChanges.length ? <button type="button" className="studio-discard-link" onClick={() => void discard()}>Отменить изменения</button> : null}<div className="studio-attention"><AlertTriangle size={15} /> Требуют внимания: 5</div></div>
  ...
  </section>{selected ? <LessonInspector lesson={selected} api={api} groups={groups} teachers={teachers} students={students} subjects={subjectOptions} saveStatus={saveStatus} onClose={() => setSelectedId('')} onSave={saveLesson} onDelete={deleteLesson} /> : null}{showVersions ? <VersionPanel layer={layer} onClose={() => setShowVersions(false)} /> : null}{showImport ? <ImportReview api={api} onClose={() => setShowImport(false)} onApply={applyImport} /> : null}{showPublish ? <div className="studio-modal-backdrop"><section className="studio-publish-modal"><header><div><p>ПУБЛИКАЦИЯ ВЕРСИИ</p><h2>{layer === 'template' ? 'Новая версия шаблона' : 'Новая версия недели'}</h2></div><button type="button" className="studio-icon-button" onClick={() => setShowPublish(false)}><X size={19} /></button></header><div className="studio-publish-summary"><strong>Проверьте изменения перед публикацией</strong><span>Изменено занятий: {draftChanges.filter((item) => item.operation === 'upsert').length}</span><span>Добавлено: {draftChanges.filter((item) => item.change_kind === 'added').length}</span><span>Удалено: {draftChanges.filter((item) => item.operation === 'delete').length}</span><span>Изменений аудитории: {draftChanges.filter((item) => item.audience_changed).length}</span><p>После публикации версия станет неизменяемой, а черновик очистится.</p></div><footer><button type="button" className="studio-secondary-button" onClick={() => setShowPublish(false)}>Продолжить редактирование</button><button type="button" className="studio-primary-button" onClick={() => void publish()}><Save size={16} /> Опубликовать версию</button></footer></section></div> : null}</div>;
}
