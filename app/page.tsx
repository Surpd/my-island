...
function TeacherApp({ data, onLogout, onReload }: { data: TeacherData; onLogout: () => void; onReload: () => void }) {
  ...
  return <main className="admin-app teacher-app"><header className="admin-header"><div><p className="eyebrow">Мой Остров · преподаватель</p><h1>Рабочий контур</h1></div><div><button className="outline-button" onClick={onReload}><RefreshCw size={15} /> Обновить</button><button className="icon-button icon-button--dark" onClick={onLogout} aria-label="Выйти"><LogOut size={17} /></button></div></header><div className="admin-grid"><section className="admin-card admin-card--wide"><div className="admin-card-heading"><div><span className="admin-icon admin-icon--blue"><BookOpen size={18} /></span><h2>Доступные курсы</h2></div><span className="count-badge">{data.courses.length}</span></div>{data.courses.length ? data.courses.map((course, index) => <div className="claim-row" key={displayValue(course.id, String(index))}><div><strong>{displayValue(course.title, 'Курс')}</strong><span>{displayValue(course.group_name, 'Подтверждённый учебный контур')}</span></div></div>) : <EmptyState icon={BookOpen} title="Курсы ещё не связаны" detail="После подтверждения teacher identity здесь появятся только авторизованные курсы." />}</section><section className="admin-card admin-card--wide"><div className="admin-card-heading"><div><span className="admin-icon admin-icon--gold"><CalendarDays size={18} /></span><h2>Расписание курсов</h2></div><span className="count-badge">{lessons.length}</span></div>{lessons.length ? <div className="lesson-stack">{lessons.map((lesson) => <LessonRow key={`${lesson.lesson_date}-${lesson.start_time}-${lesson.subject}-${lesson.source_coordinate}`} lesson={lesson} />)}</div> : <EmptyState icon={CalendarDays} title="Расписание не найдено" detail="Нет записей в разрешённом teacher scope." />}</section></div><p className="admin-footer"><LockKeyhole size={14} /> Данные ограничены курсами и группами, связанными с этой teacher identity.</p></main>;
}
