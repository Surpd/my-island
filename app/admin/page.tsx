...
function routeQuery() {
  ...
  const hashRoute = window.location.hash.replace(/^#/, '');
  const query = hashRoute.includes('?') ? hashRoute.slice(hashRoute.indexOf('?') + 1) : window.location.search.slice(1);
...
const ruLabels: Record<string, string> = {
  ...
  class: 'класс', exam_track: 'экзаменационный профиль',
  ...
  official_import: 'официальный импорт', admin_override: 'ручное изменение', spreadsheet: 'таблица', classroom: 'Классрум',
...
function readableGroupName(value: unknown) {
  if (!value) return '—';
  if (typeof value === 'object' && value !== null) {
    const item = value as RecordValue;
    if (item.display_name) return String(item.display_name);
    if (item.group_name) return readableGroupName(item.group_name);
    if (item.name) return readableGroupName(item.name);
    return '—';
  }
  const text = String(value);
  if (text.startsWith('selection:')) {
    const [, grade, ...subject] = text.split(':');
    return `Выбор предмета: ${subject.join(':') || 'не указан'}${grade ? ` · ${grade} класс` : ''}`;
  }
  if (text.startsWith('class:')) return `Класс: ${text.slice(6)}`;
  if (text.startsWith('subject:')) return `Учебная группа: ${text.slice(8).replaceAll(':', ' · ')}`;
  const gradeKey = text.match(/^grade(\d+)-([a-z]+)(?:-([a-z]))?$/i);
  if (gradeKey) {
    const subjects: Record<string, string> = { math: 'математика', english: 'английский язык', russian: 'русский язык', literature: 'литература', biology: 'биология', physics: 'физика', chemistry: 'химия', social: 'обществознание' };
    return `${gradeKey[1]} класс · ${subjects[gradeKey[2].toLowerCase()] || gradeKey[2]}${gradeKey[3] ? ` · группа ${gradeKey[3].toUpperCase()}` : ''}`;
  }
  return text;
}

function readableValue(value: unknown, field?: string) {
  if (field === 'name' || field === 'group_name' || field === 'group') return readableGroupName(value);
  if (field === 'source' || field === 'status' || field === 'mode' || field === 'action' || field === 'classification') return statusLabel(value);
...
function Overview({ navigate }: { navigate: (href: string) => void }) {
  ...
  const cards = [
    { label: 'Канонические ученики', value: overview.canonical_students, hint: 'основные записи', href: '/admin/people?kind=student' },
    { label: 'Канонические учителя', value: overview.canonical_teachers, hint: 'основные записи', href: '/admin/people?kind=teacher' },
    { label: 'Аккаунты приложения', value: overview.app_accounts, hint: 'Telegram-аккаунты', href: '/admin/people' },
    { label: 'Основные группы', value: overview.groups, hint: 'активный каталог', href: '/admin/groups' },
    { label: 'Активные проблемы', value: overview.active_issues, hint: 'требуют решения', href: '/admin/issues?status=open' },
    { label: 'Нерешённые изменения', value: overview.candidate_changes_unresolved, hint: 'кандидаты из источников', href: '/admin/candidates?status=unresolved' },
  ];
  const google = state.system?.settings?.google || {};
  return <div className="admin-stack"><div className="admin-page-actions"><p>Канонические записи, аккаунты и состояние синхронизаций показаны раздельно.</p><button className="admin-button admin-button--quiet" onClick={load}><RefreshCw size={15} /> Обновить</button></div><section className="admin-metric-grid">{cards.map((card) => <button className="admin-metric admin-metric--clickable" key={card.label} onClick={() => navigate(card.href)}><span>{card.label}</span><strong>{display(card.value, '0')}</strong><small>{card.hint}</small></button>)}</section><section className="admin-panel"><div className="admin-panel__heading"><div><p className="admin-eyebrow">ТРЕБУЕТ ВНИМАНИЯ</p><h2>Открытые состояния</h2></div><span>{display(overview.unlinked_accounts, '0')} непривязанных аккаунтов</span></div><div className="admin-chip-row"><button className="admin-link" onClick={() => navigate('/admin/issues?status=open')}>Проблемы: {display(overview.active_issues, '0')} <ChevronRight size={14} /></button><button className="admin-link" onClick={() => navigate('/admin/candidates?status=unresolved')}>Нерешённые изменения: {display(overview.candidate_changes_unresolved, '0')} <ChevronRight size={14} /></button><button className="admin-link" onClick={() => navigate('/admin/people?unlinked=true')}>Непривязанные аккаунты: {display(overview.unlinked_accounts, '0')} <ChevronRight size={14} /></button><button className="admin-link" onClick={() => navigate('/admin/sources')}>Источники: {display(overview.source_count, '0')} · запусков: {display(overview.sync_runs_total, '0')} <ChevronRight size={14} /></button></div></section><div className="admin-two-col"><section className="admin-panel"><div className="admin-panel__heading"><div><p className="admin-eyebrow">ШКОЛЬНЫЕ ДАННЫЕ</p><h2>Источники</h2></div><button className="admin-link" onClick={() => navigate('/admin/sources')}>Открыть состояние <ChevronRight size={14} /></button></div>{state.sources?.length ? <div className="admin-list">{state.sources.slice(0, 6).map((source) => <button className="admin-list-row" key={source.id} onClick={() => navigate(`/admin/sources/${source.id}`)}><span><strong>{display(source.display_name)}</strong><small>{statusLabel(source.source_type)} · {formatDate(source.last_refresh)}</small></span><Status value={source.health} /></button>)}</div> : <Empty>Источники ещё не зарегистрированы</Empty>}<div className="admin-callout admin-callout--muted"><FileSearch size={16} /><span>Последняя успешная синхронизация: {formatDate(overview.last_successful_sync)}.</span></div></section><section className="admin-panel"><div className="admin-panel__heading"><div><p className="admin-eyebrow">СВЕРКА ДАННЫХ</p><h2>Последнее проверенное состояние</h2></div><button className="admin-link" onClick={() => navigate('/admin/reconciliation')}>Открыть сверку <ChevronRight size={14} /></button></div>{state.reconciliation?.payload ? <div className="admin-inline-stats">{['KEEP', 'CREATE', 'DEACTIVATE', 'PROTECTED', 'UNRESOLVED'].map((key) => <div key={key}><strong>{display(summary[key], '0')}</strong><span>{statusLabel(key)}</span></div>)}</div> : <Empty>Пробный запуск ещё не выполнялся из панели</Empty>}<div className="admin-callout admin-callout--muted"><ShieldCheck size={16} /><span>Последний статус: {statusLabel(overview.last_reconciliation_status)} · {formatDate(overview.last_reconciliation_at)}. Запись в production из этого экрана не выполняется автоматически.</span></div></section></div><section className="admin-panel"><div className="admin-panel__heading"><div><p className="admin-eyebrow">ПОДКЛЮЧЕНИЯ</p><h2>Google и production DB</h2></div><button className="admin-link" onClick={() => navigate('/admin/system')}>Подробнее <ChevronRight size={14} /></button></div><div className="admin-system-strip"><Status value={state.system?.health?.schema_status} /><span>{statusLabel(state.system?.health?.environment)}</span><span>{state.system?.settings?.database_configured ? 'production Postgres подключён' : 'локальная база данных'}</span><span>Google: {display(google.message, 'состояние не указано')}</span></div></section></div>;
}

function People({ navigate }: { navigate: (href: string) => void }) {
  const [items, setItems] = useState<RecordValue[]>([]);
  const [filters, setFilters] = useState<{ classes: RecordValue[]; groups: RecordValue[] }>({ classes: [], groups: [] });
  ...
  const load = () => { setState({ loading: true }); const params = new URLSearchParams(); if (query) params.set('q', query); if (kind) params.set('kind', kind); if (className) params.set('class_name', className); if (groupId) params.set('group_id', groupId); if (issueFilter) params.set('has_issues', issueFilter); if (protectedFilter) params.set('protected', protectedFilter); api<{ items: RecordValue[] }>(`/api/admin/people?${params}`).then((result) => { setItems(result.items); setState({ loading: false }); }).catch((error) => setState({ loading: false, error })); };
  useEffect(() => { const params = routeQuery(); setQuery(params.get('q') || ''); setKind(params.get('kind') || ''); setClassName(params.get('class_name') || ''); setGroupId(params.get('group_id') || ''); setIssueFilter(params.get('has_issues') || ''); setProtectedFilter(params.get('protected') || ''); api<{ classes: RecordValue[]; groups: RecordValue[] }>('/api/admin/people/filters').then(setFilters).catch(() => undefined); }, []);
  useEffect(() => { const timer = window.setTimeout(load, 220); return () => window.clearTimeout(timer); }, [query, kind, className, groupId, issueFilter, protectedFilter]);
  return <div className="admin-stack"><div className="admin-toolbar"><label className="admin-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Поиск по имени, классу или группе" /></label><select value={kind} onChange={(event) => setKind(event.target.value)}><option value="">Все роли</option><option value="student">Ученики</option><option value="teacher">Учителя</option></select><select value={className} onChange={(event) => setClassName(event.target.value)}><option value="">Все классы</option>{filters.classes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><select value={groupId} onChange={(event) => setGroupId(event.target.value)}><option value="">Все группы</option>{filters.groups.map((item) => <option key={item.id} value={item.id}>{readableGroupName(item)}</option>)}</select><select value={issueFilter} onChange={(event) => setIssueFilter(event.target.value)}><option value="">Проблемы: любые</option><option value="true">Только с активными проблемами</option><option value="false">Без активных проблем</option></select><select value={protectedFilter} onChange={(event) => setProtectedFilter(event.target.value)}><option value="">Защита: любая</option><option value="true">Только защищённые случаи</option><option value="false">Без защищённого случая</option></select></div>{state.loading ? <Loading /> : state.error ? <ErrorState error={state.error} onRetry={load} /> : <section className="admin-panel admin-table-panel"><div className="admin-table-meta"><span>{items.length} записей</span><span>фильтры применяются на сервере</span></div>{items.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Человек</th><th>Тип / класс</th><th>Аккаунт</th><th>Группы</th><th>Проблемы</th></tr></thead><tbody>{items.map((person) => <tr key={person.id} onClick={() => navigate(`/admin/people/${person.id}`)}><td><strong>{display(person.display_name)}</strong><small>ID {display(person.id)}{person.protected_case ? ' · защищённый случай' : ''}</small></td><td><span>{statusLabel(person.kind)}</span><small>{display(person.class_name)}</small></td><td><Status value={person.account_status || (person.user_id ? 'linked' : 'no account')} tone={person.user_id ? 'good' : 'warn'} /></td><td><span>{Array.isArray(person.instructional_memberships) ? person.instructional_memberships.length : display(person.groups?.length, '0')}</span><small>{person.base_classes?.map((item: RecordValue) => readableGroupName(item)).join(', ') || 'нет базового класса'}</small></td><td>{person.unresolved_count ? <Status value={`${person.unresolved_count} открыто`} tone="warn" /> : <Status value="clean" tone="good" />}</td></tr>)}</tbody></table></div> : <Empty>Люди не найдены</Empty>}</section>}</div>;
}
...
function Issues({ navigate }: { navigate: (href: string) => void }) {
  const [status, setStatus] = useState(routeQuery().get('status') || 'open');
  const [issueType, setIssueType] = useState('');
  const [state, setState] = useState<{ loading: boolean; error?: unknown; items?: RecordValue[] }>({ loading: true });
  const load = () => { setState({ loading: true }); const params = new URLSearchParams({ status }); if (issueType) params.set('issue_type', issueType); api<{ items: RecordValue[] }>(`/api/admin/issues?${params}`).then((result) => setState({ loading: false, items: result.items })).catch((error) => setState({ loading: false, error })); };
  useEffect(load, [status, issueType]);
  if (state.loading) return <Loading />;
  if (state.error) return <ErrorState error={state.error} onRetry={load} />;
  const items = state.items || [];
  return <div className="admin-stack"><div className="admin-page-actions"><p>Активные проблемы требуют решения. Решённые случаи сохраняются в истории и не считаются текущими.</p><div className="admin-button-row"><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="open">Только открытые</option><option value="resolved">Только решённые</option><option value="ignored">Только пропущенные</option></select><button className="admin-button admin-button--quiet" onClick={load}><RefreshCw size={15} /> Обновить</button></div></div><section className="admin-panel admin-table-panel"><div className="admin-table-meta"><span>{items.length} проблем</span><span>без автоматического исправления</span></div>{items.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Тип</th><th>Статус</th><th>Источник</th><th>Синхронизация</th><th>Описание</th><th>Создано</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><strong>{statusLabel(item.issue_type)}</strong><small>№ {display(item.id)}</small></td><td><Status value={item.status} tone={item.status === 'open' ? 'warn' : 'neutral'} /></td><td>{display(item.source_name)}<small>{display(item.source_record_key)}</small></td><td>{statusLabel(item.sync_status)}<small>{formatDate(item.sync_finished_at)}</small></td><td className="admin-json-cell">{display(item.details)}</td><td>{formatDate(item.created_at)}<small>{item.resolved_at ? `решено: ${formatDate(item.resolved_at)}` : ''}</small></td></tr>)}</tbody></table></div> : <Empty>Проблем такого статуса нет</Empty>}</section></div>;
}

function Candidates() {
  const [status, setStatus] = useState(routeQuery().get('status') || '');
  const [state, setState] = useState<{ loading: boolean; error?: unknown; items?: RecordValue[] }>({ loading: true });
  const load = () => { setState({ loading: true }); const params = new URLSearchParams(); if (status) params.set('status', status); api<{ items: RecordValue[] }>(`/api/admin/candidates?${params}`).then((result) => setState({ loading: false, items: result.items })).catch((error) => setState({ loading: false, error })); };
  useEffect(load, [status]);
  if (state.loading) return <Loading />;
  if (state.error) return <ErrorState error={state.error} onRetry={load} />;
  const items = state.items || [];
  return <div className="admin-stack"><div className="admin-page-actions"><p>Кандидаты источников — это история изменений и результаты синхронизаций. Здесь ничего не применяется в канонические данные.</p><div className="admin-button-row"><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Все статусы</option><option value="unresolved">Нерешённые</option><option value="pending">Ожидающие</option><option value="conflict">Конфликты</option><option value="applied">Применённые (история)</option></select><button className="admin-button admin-button--quiet" onClick={load}><RefreshCw size={15} /> Обновить</button></div></div><section className="admin-panel admin-table-panel"><div className="admin-table-meta"><span>{items.length} записей</span><span>без записи в базу из этого раздела</span></div>{items.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Статус</th><th>Сущность</th><th>Изменение</th><th>Источник / запуск</th><th>Естественный ключ</th><th>Время</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><Status value={item.status} tone={item.status === 'unresolved' || item.status === 'conflict' ? 'warn' : item.status === 'applied' ? 'neutral' : 'good'} /></td><td>{statusLabel(item.entity_type)}<small>№ {display(item.id)}</small></td><td>{statusLabel(item.change_type)}</td><td>{display(item.source_name)}<small>запуск № {display(item.sync_run_id)} · {statusLabel(item.sync_status)}</small></td><td className="admin-mono">{display(item.natural_key)}</td><td>{formatDate(item.created_at)}<small>{item.applied_at ? `применено: ${formatDate(item.applied_at)}` : ''}</small></td></tr>)}</tbody></table></div> : <Empty>Изменений такого статуса нет</Empty>}</section></div>;
}
...
function Schedule() {
  ...
  const values = (key: string) => [...new Set(state.lessons.map((item) => String(item[key] ?? '')).filter(Boolean))];
  const text = (item: RecordValue, ...keys: string[]) => display(keys.map((key) => item[key]).find((value) => value !== undefined && value !== null && value !== ''), '—');
  const json = (value: unknown) => typeof value === 'object' && value !== null ? JSON.stringify(value, null, 2) : display(value);
  const lessonStatus = (item: RecordValue) => item.status || item.issue_status || item.parse_status || 'RESOLVED';
  const teacherOptions = [...new Map(state.lessons.filter((item) => item.teacher_id).map((item) => [String(item.teacher_id), String(item.teacher_name || item.teacher_id)]))];
  const groupOptions = [...new Map(state.lessons.filter((item) => item.group_id).map((item) => [String(item.group_id), String(item.group_name || item.audience || item.group_id)]))];
  const tabs = (Array.isArray(overview.tabs) ? overview.tabs : []).map((item: any) => typeof item === 'object' ? `${item.title || item.sheet_id || 'Лист'}${item.week_start ? ` · ${item.week_start}–${item.week_end || ''}` : ''}` : String(item));
  const weeks: Array<{ value: string; label: string }> = (Array.isArray(overview.weeks) ? overview.weeks : []).map((item: any) => typeof item === 'object' ? { value: String(item.week_start), label: String(item.label || item.week_start) } : { value: String(item), label: String(item) });
  return <div className="admin-stack"><div className="admin-page-actions"><p>Оперативный контроль источника, среза недели и расхождений расписания.</p><div className="admin-button-row"><button className="admin-button admin-button--quiet" disabled={busy} onClick={() => void refresh()}><RefreshCw size={15} className={busy ? 'admin-spin' : ''} /> {busy ? 'Обновляем…' : 'Обновить из источника'}</button><button className="admin-button admin-button--quiet" onClick={() => load(week)}>Перечитать</button></div></div>{action && <div className="admin-callout admin-callout--safe"><CheckCircle2 size={16} />{action}</div>}{overview.auth_state && <div className="admin-callout admin-callout--muted"><ShieldCheck size={16} /><span>Авторизация источника: {statusLabel(overview.auth_state)}{overview.auth_state !== 'connected' ? ' — требуется действие администратора.' : ''}</span></div>}<section className="admin-panel"><div className="admin-panel__heading"><div><p className="admin-eyebrow">ИСТОЧНИК / ШАБЛОН</p><h2>{text(source, 'name', 'title', 'spreadsheet_title')}</h2></div><Status value={source.status || overview.auth_state || 'connected'} /></div><div className="admin-chip-row"><span>Шаблон: {text(source, 'template', 'template_name')}</span><span>Вкладки: {tabs.length ? tabs.join(', ') : '—'}</span><span>Неделя: {week || '—'}</span><span>Последняя синхронизация: {formatDate(overview.last_sync)}</span></div></section><section className="admin-inline-stats">{[['RESOLVED','resolved'],['WARNING','warning'],['UNRESOLVED','unresolved'],['CONFLICT','conflict'],['Добавлено','added'],['Изменено','changed'],['Удалено','removed']].map(([label, key]) => <div key={key} className={['warning','unresolved','conflict'].includes(key) ? 'is-warning' : ''}><strong>{display(summary[key] ?? summary[label], '0')}</strong><span>{label}</span></div>)}</section><div className="admin-toolbar admin-schedule-filters"><select value={week} onChange={(event) => { setWeek(event.target.value); load(event.target.value); }}><option value="">Все недели</option>{weeks.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Все статусы</option><option value="RESOLVED">RESOLVED</option><option value="WARNING">WARNING</option><option value="UNRESOLVED">UNRESOLVED</option><option value="CONFLICT">CONFLICT</option></select><select value={teacher} onChange={(event) => setTeacher(event.target.value)}><option value="">Все учителя</option>{teacherOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={group} onChange={(event) => setGroup(event.target.value)}><option value="">Все классы / группы</option>{groupOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={lessonType} onChange={(event) => setLessonType(event.target.value)}><option value="">Все типы урока</option>{values('lesson_type').map((item) => <option key={item} value={item}>{item}</option>)}</select></div><div className="admin-schedule-workspace"><section className="admin-panel admin-table-panel"><div className="admin-table-meta"><span>{state.lessons.length} уроков</span><span>{state.issues.length} проблем</span></div>{state.lessons.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Дата / время</th><th>Предмет</th><th>Учитель</th><th>Класс / группа</th><th>Статус</th></tr></thead><tbody>{state.lessons.map((item, index) => <tr key={item.id || item.lesson_id || index} onClick={() => setSelected(item)}><td><strong>{text(item, 'lesson_date','date')}</strong><small>{text(item,'start_time','time')}–{text(item,'end_time')}</small></td><td>{text(item,'subject')}</td><td>{text(item,'teacher_name','teacher')}</td><td>{text(item,'group_name','group','audience')}</td><td><Status value={lessonStatus(item)} tone={lessonStatus(item) === 'RESOLVED' ? 'good' : lessonStatus(item) === 'CONFLICT' || lessonStatus(item) === 'UNRESOLVED' ? 'bad' : 'warn'} /></td></tr>)}</tbody></table></div> : <Empty>Уроков по выбранным фильтрам нет</Empty>}</section>{selected && <section className="admin-panel admin-schedule-detail"><div className="admin-panel__heading"><div><p className="admin-eyebrow">ДЕТАЛИ УРОКА</p><h2>{text(selected,'subject')} · {text(selected,'lesson_date','date')}</h2></div><button className="admin-link" onClick={() => setSelected(null)}>Закрыть</button></div><div className="admin-detail-list">{[['raw_text','Исходный текст'],['cell','Ячейка'],['color','Цвет'],['parsed','Разобрано'],['resolved','Разрешено'],['evidence','Свидетельства'],['confidence','Уверенность'],['baseline','Базовая версия'],['diff','Изменение'],['issue','Проблема']].map(([key,label]) => <div key={key}><strong>{label}</strong><small>{json(selected[key] ?? (key === 'raw_text' ? selected.raw_source : undefined))}</small></div>)}</div></section>}</div><section className="admin-panel"><div className="admin-panel__heading"><h2>Проблемы недели</h2><span>{state.issues.length}</span></div>{state.issues.length ? <div className="admin-detail-list">{state.issues.slice(0, 50).map((item, index) => <div key={item.id || index}><strong>{text(item,'title','subject','issue_type')}</strong><small>{statusLabel(text(item,'status','severity'))} · {json(item.detail ?? item.details ?? item.reason ?? item.message)}</small></div>)}</div> : <Empty>Проблем не обнаружено</Empty>}</section></div>;
}
...
function System({ navigate }: { navigate: (href: string) => void }) {
  const [state, setState] = useState<{ loading: boolean; error?: unknown; data?: RecordValue }>({ loading: true });
  ...
  useEffect(load, []); if (state.loading) return <Loading />; if (state.error) return <ErrorState error={state.error} onRetry={load} />;
  const data = state.data || {}; const countLinks: Record<string, string> = { users: '/admin/people', identities: '/admin/people', groups: '/admin/groups', memberships: '/admin/groups', school_sources: '/admin/sources', school_sync_runs: '/admin/sources', school_candidate_changes: '/admin/candidates', school_resolution_issues: '/admin/issues', audit_log: '/admin/audit' }; const countLabels: Record<string, string> = { users: 'аккаунты приложения', identities: 'канонические личности', groups: 'основные группы', memberships: 'активные членства', school_sources: 'источники школы', school_sync_runs: 'запуски синхронизации', school_candidate_changes: 'кандидаты изменений', school_resolution_issues: 'проблемы разрешения', audit_log: 'события аудита' }; const google = data.settings?.google || {}; return <div className="admin-stack"><div className="admin-page-actions"><p>Среда, соответствие схемы и количества записей. Секреты намеренно не отображаются.</p><button className="admin-button admin-button--quiet" onClick={load}><RefreshCw size={15} /> Обновить</button></div><section className="admin-panel"><div className="admin-panel__heading"><h2>Состояние сервера</h2><Status value={data.health?.schema_status} /></div><dl className="admin-definition-list"><dt>Среда</dt><dd>{statusLabel(data.health?.environment)}</dd><dt>База данных</dt><dd>{statusLabel(data.settings?.database_configured ? 'configured' : 'local')}</dd><dt>Разработка</dt><dd>{data.settings?.dev_auth_enabled ? 'включена' : 'выключена'}</dd><dt>Google</dt><dd><Status value={google.status} /> · {display(google.message)}</dd></dl></section><section className="admin-panel"><h2>Количество записей</h2><div className="admin-inline-stats">{Object.entries(data.health?.counts || {}).map(([key, value]) => countLinks[key] ? <button className="admin-inline-stat admin-inline-stat--clickable" key={key} onClick={() => navigate(countLinks[key])}><strong>{String(value)}</strong><span>{countLabels[key] || key}</span></button> : <div key={key}><strong>{String(value)}</strong><span>{countLabels[key] || key}</span></div>)}</div><div className="admin-callout admin-callout--muted"><Database size={16} /><span>Исторические записи изменений и активные проблемы разделены: {display(data.health?.semantics?.candidate_changes?.historical, '0')} применённых исторических записей, {display(data.health?.semantics?.candidate_changes?.active, '0')} активных кандидатов; открытых проблем: {display(data.health?.semantics?.resolution_issues?.active, '0')}.</span></div></section><section className="admin-panel"><h2>Проверки схемы</h2><div className="admin-detail-list">{Object.entries(data.health?.schema || {}).map(([key, value]: [string, any]) => <div key={key}><strong>{key}</strong><small><Status value={value.ok ? 'current' : 'attention'} /> · требуется: {value.required.join(', ')}</small></div>)}</div></section></div>;
}
...
function AppContent({ path, navigate }: { path: string; navigate: (href: string) => void }) {
  ...
  if (path === '/admin/issues') return <Issues navigate={navigate} />;
  if (path === '/admin/candidates') return <Candidates />;
