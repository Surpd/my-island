...
It resolves only existing canonical teacher identities, subjects, and groups; it does not create entities, change teacher assignments, or touch student memberships. `Весь класс` maps to existing class cohorts, Math 9 A/B/C and global English groups remain explicit, 10–11 shared lessons retain separate grade-specific assignments, and computed audiences such as Grade 11 Geography exclude Society EGE without becoming a membership group. The initial live preview is preserved in the two `TEACHER_DIRECTORY_DRY_RUN_2026-09-08.*` artifacts; it had one unresolved audience (Literature 11 Base).

## Structured teacher-directory apply (2026-09-08)

The confirmed Literature 11 Base audience was created as `instructional:литература:11:база`. Its seven memberships were derived only from the canonical Grade 11 class roster minus the six canonical Literature EGE memberships; no existing membership was rebuilt or removed. The guarded apply then reconciled the structured teacher source atomically: 12 assignments were created, 6 metadata rows updated, 12 obsolete assignments were deactivated with `valid_until`, and no records were physically deleted. The final read-back is stored in `TEACHER_DIRECTORY_RECONCILIATION_FINAL_2026-09-08.md` and `.json`: 125 active teacher assignments, 24 active teachers, 488 active student memberships, 0 duplicate active assignments, 0 unresolved issues, and 0 conflicts. Computed Grade 11 Geography remains a schedule audience, not a permanent teacher assignment.
