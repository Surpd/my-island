...
The live reconciliation confirmed the 2026/27 source authority: base classes come from `Списки по классам 26/27`, instructional and English rosters from `списки групп 26-27`, and OGE/EGE/profile memberships from `ОГЭ/ЕГЭ`. The school has separate base classes 9-А and 9-Д; Grade 9 Math is the distinct canonical A/B/C subject structure (not a universal naming scheme). The parser carries merged grade headings and does not terminate a roster merely because one column has an empty row. Teacher labels in rosters are not treated as canonical teachers unless a full identity is proven. Source disappearance ends source-derived relationships and can inactivate an otherwise unlinked imported identity without deleting its history.

Initial bootstrap may use Groq for ambiguous semantic units, but Groq is behind a provider/model-neutral interface (`GroqSemanticProvider`) and never writes canonical rows. The identity resolver first applies exact/normalization, confirmed source observations, and surname + short-name + context matching; only a small remaining candidate set may reach Groq, and its proposal still needs deterministic validation. The last known valid snapshot remains explicit. Source spellings and cell-level observations are persisted through `school_source_mappings`.
